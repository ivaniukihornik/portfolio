...
class BasePage:
    ...
    def __wait_until_element_visible(self, locator, mode=''):
        match mode:
            case 'fast':
                return self.__fast_wait.until(EC.visibility_of_element_located(locator))
            case _:
    ...
    def _make_screenshot(self, name='Screenshot', with_hard_wait=False):
        if with_hard_wait:
            self.__hard_wait(0.5)
        return allure.attach(self.driver.get_screenshot_as_png(), name, allure.attachment_type.PNG)
